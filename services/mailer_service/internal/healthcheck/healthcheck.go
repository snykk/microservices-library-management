...
// PerformHealthCheck checks the status of RabbitMQ connection.
func PerformHealthCheck(rabbitConn *amqp.Connection) bool {
...
func StartHealthCheckServer(shutdownContext context.Context, rabbitConn *amqp.Connection) {
	// Create a new ServeMux for routing
	mux := http.NewServeMux()

	// Handle the health check route
	mux.HandleFunc("/healthy", func(w http.ResponseWriter, r *http.Request) {
		healthy := PerformHealthCheck(rabbitConn)
		if !healthy {
